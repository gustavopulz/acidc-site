...
async function main() {
  ...
  // ─── Admin user ───────────────────────────────────────────────
  const email    = process.env.ADMIN_EMAIL    ?? "admin@acidc.com.br";
  const password = process.env.ADMIN_PASSWORD ?? "acidc@admin2024";

  const existing = await prisma.admin.findUnique({ where: { email } });
  if (!existing) {
    const hash = await bcrypt.hash(password, 12);
    await prisma.admin.create({ data: { email, password: hash, name: "Admin" } });
  ...
  if (showCount === 0) {
    await prisma.show.createMany({
      data: [
        { date: "11/04/2026", city: "Leme - SP",      venue: "Independente MC", status: "tickets", link: "https://www.instagram.com/independentemotoclube/" },
        { date: "20/06/2026", city: "Leme - SP",      venue: "Kaiowas MC",      status: "tickets", link: "https://www.instagram.com/kaiowasmotoclube/" },
        { date: "28/11/2026", city: "Ibitinga - SP",  venue: "Alta Voltagem MC", status: "tickets", link: "https://www.instagram.com/altavoltagemmc/" },
      ],
    });
    console.log("✅ Shows seeded");
  } else {
    console.log(`ℹ️  ${showCount} shows already exist, skipping`);
  }
  ...
  if (albumCount === 0) {
    type AlbumSeed = {
      ...
      date: string;
      folder: string;
      photos: { url: string; pinned: boolean }[];
      videos: { src: string; title?: string; pinned: boolean }[];
    };
    ...
    const albumsData: AlbumSeed[] = [
      {
        ...
        photos: [],
        videos: [
          { src: "/uploads/cali-22-11-2025/1.mp4", pinned: true  },
          { src: "/uploads/cali-22-11-2025/2.mp4", pinned: true  },
          { src: "/uploads/cali-22-11-2025/3.mp4", pinned: true  },
          { src: "/uploads/cali-22-11-2025/4.mp4", pinned: false },
          { src: "/uploads/cali-22-11-2025/5.mp4", pinned: false },
          { src: "/uploads/cali-22-11-2025/6.mp4", pinned: false },
          { src: "/uploads/cali-22-11-2025/7.mp4", pinned: false },
        ],
      },
      {
        ...
        photos: [{ url: "/uploads/zapata-15-11-2025/foto-capa.jpeg", pinned: true }],
        videos: [{ src: "/uploads/zapata-15-11-2025/video.mp4", pinned: true }],
      },
      {
        ...
        ].map((url, i) => ({ url, pinned: i < 2 })),
        videos: [{ src: "/uploads/rock-in-country-18-10-2025/VID_20251026_124254_315.mp4", pinned: true }],
      },
      {
        title: "Hangar 111",
        date: "04/10/2025",
    ...
    for (const a of albumsData) {
      await prisma.album.create({
        data: {
          ...
          date:  a.date,
          folder: a.folder,
          photos: { create: a.photos },
          videos: { create: a.videos },
        },
      });
    }
    console.log(`✅ ${albumsData.length} albums seeded`);
  } else {
    console.log(`ℹ️  ${albumCount} albums already exist, skipping`);
  }
